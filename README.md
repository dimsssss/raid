...
예상치 못하게 낙관적락을 그대로 구현할 수 없었다. 사용하는 오픈소스에서 설명한대로 설정을 했지만 제대로 동작을 하지 않았다.(version이라는 속성을 주면 경합 시 특정 예외를 발생시킨다고 하였는데 그 특정 예외를 발생시키지 않았다)

그래서 직접 잠금처리를 하게 되었다. 요구 사항으로 오직 한명만 보스 레이드를 시작할 수 있고 3분 동안 누구도 레이드를 진행할 수 없다
